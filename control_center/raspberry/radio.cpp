...
string RXs = "";
int RXbytes = 0, RXpages = 0, RXtail = 0, nameLength = 0, namePages = 0, checksum = 0, arrPos = 0;
byte *tempArray;
...
{
	byte dat[32];
	for (int i = 0; i < 8; i++) dat[i] = marker[i];

	int Length = msg.length();
	int Pages = Length / 32; if (Length % 32 > 0) Pages++;
...
bool SendFile(string patch, string sendname)
{
	if (access(patch.c_str(), 0) == -1) return false;
	byte dat[32];
	for (int i = 0; i < 8; i++) dat[i] = marker[i];
	byte array[2097152];
	...
	int length = fread(array, sizeof(byte), 2097152, file);
	fclose(file);
	...
	int cs = 0;
	for (int i = 0; i < length; i++) cs += array[i];
	cs = cs % (256 * 256);
	int namePages = sendname.length() / 32; if (sendname.length() % 32 > 0) namePages++;
	// 1b длинна имени // 1b пакетов с именем // 2b checksum
	dat[12] = (byte)(sendname.length());
	...
	for (int i = 0; i < namePages; i++)
	{
		for (int j = i * 32; j < (i + 1) * 32; j++)
		{
			if (j < int(sendname.length())) dat[j - i * 32] = (byte)sendname[j];
			else dat[j - i * 32] = 0;
		}
		sendPacket(&dat[0]);
	}
...
void setLag(unsigned int lagtime)
{
	lag = clag * lagtime;
}